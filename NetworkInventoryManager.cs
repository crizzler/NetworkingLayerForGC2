...
namespace Arawn.GameCreator2.Networking.Inventory
{
    ...
    public class NetworkInventoryManager : NetworkSingleton<NetworkInventoryManager>
    {
        ...
        // [LOCAL-EDIT] #PILFER-INVENTORY-SERVER-LOOT
        public Action<NetworkLootRequest> OnSendLootRequest;
        ...
        // [LOCAL-EDIT] #PILFER-INVENTORY-SERVER-LOOT
        public Action<uint, NetworkLootResponse> OnSendLootResponse;
        ...
        public void ReceiveTransferRequest(NetworkTransferRequest request, ulong clientId)
        {
            ...
            // TODO: Support authorized world/container-to-world/container reorganization once container access locks and concurrent looting rules are defined.
            if (sourceController != null && destinationController != null &&
                sourceController.IsWorldInventory && destinationController.IsWorldInventory)
            {
                Debug.LogWarning(
                    $"[NetworkInventoryPickupDebug][Manager] transfer rejected world-to-world deferred req={request.RequestId} sourceBag={request.SourceBagNetworkId} destinationBag={request.DestinationBagNetworkId}");
        ...
        public void ReceivePickupRequest(NetworkPickupRequest request, ulong clientId)
        {
            ...
            try
            {
                ...
                response.CorrelationId = request.CorrelationId;
                Debug.Log(
                    $"[NetworkInventoryPickupDebug][Manager] pickup processed req={request.RequestId} authorized={response.Authorized} reason={response.RejectionReason} pickupFailure={response.PickupFailure} prop={response.PropNetworkId} senderClient={senderClientId} placed={response.PlacedPosition}");
        ...
        // [LOCAL-EDIT] #PILFER-INVENTORY-SERVER-LOOT
        public void ReceiveLootRequest(NetworkLootRequest request, ulong clientId)
        {
            if (!m_IsServer) return;
            uint senderClientId = GetSenderClientId(clientId);
            Debug.Log(
                $"[NetworkInventoryLootDebug][Manager] receive loot request req={request.RequestId} senderConnection={clientId} senderClient={senderClientId} actor={request.ActorNetworkId} container={request.ContainerBagNetworkId}");

            if (!SecurityIntegration.ValidateModuleRequest(
                    senderClientId,
                    BuildContext(request.ActorNetworkId, request.CorrelationId),
                    "Inventory",
                    nameof(NetworkLootRequest)))
            {
                SendLootResponse(senderClientId, new NetworkLootResponse
                {
                    RequestId = request.RequestId,
                    ActorNetworkId = request.ActorNetworkId,
                    CorrelationId = request.CorrelationId,
                    ContainerBagNetworkId = request.ContainerBagNetworkId,
                    Authorized = false,
                    RejectionReason = GetSecurityRejection(request.ActorNetworkId, request.CorrelationId),
                    LootFailure = NetworkLootFailure.None
                });
                return;
            }
            ...
            try
            {
                NetworkInventoryController container = GetController(request.ContainerBagNetworkId);
                if (container == null)
                {
                    SendLootResponse(senderClientId, new NetworkLootResponse
                    {
                        ...
                        Authorized = false,
                        RejectionReason = InventoryRejectionReason.BagNotFound,
                        LootFailure = NetworkLootFailure.ContainerBagNotFound
                    });
                    return;
                }

                NetworkLootResponse response = container.ProcessLootRequest(request, senderClientId);
                response.ActorNetworkId = request.ActorNetworkId;
                response.CorrelationId = request.CorrelationId;
                Debug.Log(
                    $"[NetworkInventoryLootDebug][Manager] loot processed req={request.RequestId} authorized={response.Authorized} generated={response.Generated} reason={response.RejectionReason} lootFailure={response.LootFailure} senderClient={senderClientId} container={response.ContainerBagNetworkId}");
                SendLootResponse(senderClientId, response);
            }
            finally
            {
                DecrementPendingRequests(clientId);
        ...
        {
            Debug.Log(
                $"[NetworkInventoryPickupDebug][Manager] send pickup response req={response.RequestId} target={targetNetworkId} authorized={response.Authorized} reason={response.RejectionReason} pickupFailure={response.PickupFailure} prop={response.PropNetworkId} placed={response.PlacedPosition}");
            OnSendPickupResponse?.Invoke(targetNetworkId, response);
        }

        // [LOCAL-EDIT] #PILFER-INVENTORY-SERVER-LOOT
        private void SendLootResponse(uint targetNetworkId, NetworkLootResponse response)
        {
            Debug.Log(
                $"[NetworkInventoryLootDebug][Manager] send loot response req={response.RequestId} target={targetNetworkId} authorized={response.Authorized} generated={response.Generated} reason={response.RejectionReason} lootFailure={response.LootFailure} container={response.ContainerBagNetworkId}");
            OnSendLootResponse?.Invoke(targetNetworkId, response);
        }
        ...
        {
            Debug.Log(
                $"[NetworkInventoryPickupDebug][Manager] receive pickup response target={targetNetworkId} req={response.RequestId} authorized={response.Authorized} reason={response.RejectionReason} pickupFailure={response.PickupFailure} prop={response.PropNetworkId} placed={response.PlacedPosition}");

            if (!response.Authorized)
            {
                // [LOCAL-EDIT] #PILFER-INVENTORY-WORLD-OBJECT-REJECT-DIAGNOSTICS
                Debug.LogWarning($"[NetworkInventoryManager] Pickup rejected: {response.RejectionReason} pickupFailure={response.PickupFailure} prop={response.PropNetworkId}");
            }
        }

        // [LOCAL-EDIT] #PILFER-INVENTORY-SERVER-LOOT
        public void ReceiveLootResponse(NetworkLootResponse response, uint targetNetworkId)
        {
            Debug.Log(
                $"[NetworkInventoryLootDebug][Manager] receive loot response target={targetNetworkId} req={response.RequestId} authorized={response.Authorized} generated={response.Generated} reason={response.RejectionReason} lootFailure={response.LootFailure} container={response.ContainerBagNetworkId}");

            if (!response.Authorized)
            {
                Debug.LogWarning($"[NetworkInventoryManager] Loot rejected: {response.RejectionReason} lootFailure={response.LootFailure} container={response.ContainerBagNetworkId}");
            }
        }
