...
namespace Arawn.GameCreator2.Networking.Security
{
    ...
    public class NetworkSecurityManager : NetworkSingleton<NetworkSecurityManager>
    {
        ...
        {
            // The manager is scene-owned and can survive a transport stop/restart. Request
            // counters belong to the transport session, however, and restart from their
            // initial value when characters/controllers are recreated. Keeping the previous
            // replay high-water marks would therefore reject every request in the new session
            // until its counter happened to overtake the old one.
            //
            // Only reset on a real role/session transition. Several modules may redundantly
            // ensure the same server role while a session is live; those calls must not open a
            // replay window by clearing the active session's tracking state.
            bool sessionChanged = !m_IsInitialized || m_IsServer != isServer;
            if (sessionChanged)
            {
                ResetSessionTracking();
            }
            ...
            if (sessionChanged)
            {
                // Create fresh rate limiters for the new session.
