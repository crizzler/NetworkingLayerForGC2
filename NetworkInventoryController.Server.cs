    ...
    public partial class NetworkInventoryController
    {
        ...
        public NetworkPickupResponse ProcessPickupRequest(NetworkPickupRequest request, uint clientNetworkId)
        {
            ...
            if (!TryGetServerDroppedWorldItem(request.RuntimeIdHash, out ServerDroppedWorldItem droppedWorldItem))
            {
                ...
                if (TryProcessWorldObjectPickupRequest(request, clientNetworkId, out NetworkPickupResponse worldObjectResponse))
                {
        ...
        public NetworkLootResponse ProcessLootRequest(NetworkLootRequest request, uint clientNetworkId)
        {
            Debug.Log(
                $"[NetworkInventoryLootDebug] {name}: server loot request received req={request.RequestId} client={clientNetworkId} actor={request.ActorNetworkId} container={request.ContainerBagNetworkId} controllerBag={NetworkId} world={IsWorldInventory}");

            if (!m_IsServer)
            {
                return BuildLootResponse(request, false, false, InventoryRejectionReason.NotAuthorized, NetworkLootFailure.None);
            }

            if (request.ContainerBagNetworkId != NetworkId)
            {
                return BuildLootResponse(request, false, false, InventoryRejectionReason.BagNotFound, NetworkLootFailure.ContainerBagNotFound);
            }

            if (!IsWorldInventory)
            {
                return BuildLootResponse(request, false, false, InventoryRejectionReason.InvalidOperation, NetworkLootFailure.ContainerIsNotWorldInventory);
            }

            NetworkLootContainer lootContainer = GetComponent<NetworkLootContainer>() ??
                                                 GetComponentInParent<NetworkLootContainer>() ??
                                                 GetComponentInChildren<NetworkLootContainer>();
            if (lootContainer == null)
            {
                return BuildLootResponse(request, false, false, InventoryRejectionReason.InvalidOperation, NetworkLootFailure.LootContainerMissing);
            }

            if (lootContainer.LootTable == null)
            {
                return BuildLootResponse(request, false, false, InventoryRejectionReason.ItemNotFound, NetworkLootFailure.LootTableMissing);
            }

            if (lootContainer.GenerateOnce && lootContainer.HasGenerated)
            {
                BroadcastFullState();
                return BuildLootResponse(request, true, false, InventoryRejectionReason.None, NetworkLootFailure.AlreadyGenerated);
            }

            bool generated = lootContainer.LootTable.Run(m_Bag);
            if (!generated)
            {
                return BuildLootResponse(request, false, false, InventoryRejectionReason.ItemNotFound, NetworkLootFailure.LootRollFailed);
            }

            lootContainer.MarkGenerated();
            TrackAllCurrentRuntimeItemsForServerLoot();
            CacheCurrentSyncState();
            BroadcastFullState();

            int cells = GetFullSnapshot().Cells?.Length ?? 0;
            if (lootContainer.LogDiagnostics)
            {
                Debug.Log(
                    $"[NetworkInventoryLootDebug] {name}: generated server loot container={NetworkId} generated={generated} cells={cells}",
                    this);
            }

            return BuildLootResponse(request, true, true, InventoryRejectionReason.None, NetworkLootFailure.None);
        }

        // [LOCAL-EDIT] #PILFER-INVENTORY-SERVER-LOOT
        private void TrackAllCurrentRuntimeItemsForServerLoot()
        {
            foreach (Cell cell in m_Bag.Content.CellList)
            {
                if (cell == null || cell.Available) continue;

                TrackRuntimeItemRecursive(cell.RootRuntimeItem);
        ...
        // [LOCAL-EDIT] #PILFER-INVENTORY-SERVER-LOOT
        private static NetworkLootResponse BuildLootResponse(
            NetworkLootRequest request,
            bool authorized,
            bool generated,
            InventoryRejectionReason reason,
            NetworkLootFailure failure)
        {
            return new NetworkLootResponse
            {
                RequestId = request.RequestId,
