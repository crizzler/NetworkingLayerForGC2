...
namespace Arawn.GameCreator2.Networking
{
    ...
    [Title("Network Stop Gestures")]
    [Description("Stops Game Creator 2 Gestures on a Character and replicates the stop to remote clients")]

    [Category("Network/Characters/Animation/Network Stop Gestures")]
    ...
    [Keywords("Network", "Characters", "Animation", "Animate", "Gesture", "Stop")]
    [Image(typeof(IconCharacterGesture), ColorTheme.Type.Blue, typeof(OverlayCross))]

    [Serializable]
    public class InstructionNetworkCharacterStopGesture : Instruction
    {
        ...
        public override string Title => $"Network Stop gestures on {m_Character}";
        ...
        protected override Task Run(Args args)
        {
            ...
            Log(
                $"requested '{character.name}' delay={delay:F3} transition={transition:F3} " +
            ...
            {
                Log($"stopping network gestures '{character.name}'");
                animimController.StopGestures(delay, transition);
            }
            else if (m_PlayLocalFallback)
            {
                Log(
                    $"stopping local fallback '{character.name}' " +
