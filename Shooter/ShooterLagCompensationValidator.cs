    ...
    public class ShooterLagCompensationValidator : ICombatLagCompensationValidator
    {
            ...
            ShooterWeapon weapon = null)
        {
            ...
            if (!TryRefreshLagManager())
            {
                ...
                result.RejectionReason = ShotRejectionReason.ShooterNotFound;
            ...
            ShooterWeapon weapon = null)
        {
            ...
            if (!request.IsCharacterHit || request.TargetNetworkId == 0)
            {
                ...
                float environmentMaxRange = GetWeaponRange(weapon);
                ...
                if (projectedDistance > environmentMaxRange + Config.MaxDistanceDeviation)
                {
                    result.IsValid = false;
                    result.RejectionReason = CombatValidationRejectionReason.OutOfShooterRange;
                    result.RejectionDetails = $"Environment hit distance {projectedDistance:F2}m exceeds range {environmentMaxRange:F2}m";
                    return result;
                }
                ...
                float environmentDistanceDeviation = Mathf.Abs(request.Distance - Mathf.Max(0f, projectedDistance));
                if (environmentDistanceDeviation > Config.MaxDistanceDeviation)
                {
                    result.IsValid = false;
                    result.RejectionReason = CombatValidationRejectionReason.InvalidTrajectory;
                    result.RejectionDetails = $"Environment distance deviation {environmentDistanceDeviation:F2}m exceeds max";
                    return result;
                }
