...
namespace Arawn.GameCreator2.Networking.Inventory
{
    ...
    public partial class NetworkInventoryController
    {
        ...
        // [LOCAL-EDIT] #PILFER-INVENTORY-UI-TRANSFER-BRIDGE
        public void RequestTransferItem(NetworkInventoryController destination, RuntimeItem runtimeItem, Vector2Int destinationPosition, bool allowStack)
        {
            if (destination == null || runtimeItem == null) return;

            if (destination == this)
            {
                Vector2Int sourcePosition = m_Bag.Content.FindPosition(runtimeItem.RuntimeID);
                if (sourcePosition == TBagContent.INVALID) return;

                // TODO: Same world/container reorganization needs explicit access rules before client UI can request it.
                if (IsWorldInventory)
                {
                    LogPickupWarning(
                        $"{name}: same world-container move skipped until container access rules are implemented item={DescribeRuntimeItem(runtimeItem)} source={sourcePosition} destination={destinationPosition}",
                        this);
                    return;
                }

                RequestMoveItem(sourcePosition, destinationPosition, allowStack);
                return;
            }

            if (NetworkId == 0 || destination.NetworkId == 0) return;
            if (!TryGetLocalActorNetworkId(out uint actorNetworkId))
            {
                LogPickupWarning(
                    $"{name}: transfer request skipped no local actor network id sourceBag={NetworkId} destinationBag={destination.NetworkId} item={DescribeRuntimeItem(runtimeItem)}",
                    this);
                return;
            }
            ...
            LogPickupDebug(
                $"{name}: sending transfer request req={request.RequestId} actor={actorNetworkId} sourceBag={NetworkId} destinationBag={destination.NetworkId} item={DescribeRuntimeItem(runtimeItem)} destination={destinationPosition} server={m_IsServer}",
                this);

            if (m_IsServer)
            {
                NetworkTransferResponse response = ProcessTransferRequest(request, destination, NetworkId);
                response.ActorNetworkId = request.ActorNetworkId;
                response.CorrelationId = request.CorrelationId;
                NetworkInventoryManager.Instance?.ReceiveTransferResponse(response, actorNetworkId);
                return;
            }
        ...
        public void RequestLootGeneration(NetworkInventoryController containerInventory)
        {
            ...
            if (!TryGetLocalActorNetworkId(out uint actorNetworkId))
            {
                LogPickupWarning($"{name}: loot request skipped no local actor network id container={containerInventory.NetworkId}", this);
                return;
            }

            var request = new NetworkLootRequest
            {
                RequestId = GetNextRequestId(),
                ActorNetworkId = actorNetworkId,
                CorrelationId = NetworkCorrelation.Compose(actorNetworkId, m_LastIssuedRequestId),
                ContainerBagNetworkId = containerInventory.NetworkId
            };

            Debug.Log(
                $"[NetworkInventoryLootDebug] {name}: sending loot request req={request.RequestId} actor={request.ActorNetworkId} container={request.ContainerBagNetworkId} server={m_IsServer} local={m_IsLocalClient}");

            if (m_IsServer)
            {
                NetworkLootResponse response = containerInventory.ProcessLootRequest(request, NetworkId);
                response.ActorNetworkId = request.ActorNetworkId;
                response.CorrelationId = request.CorrelationId;
                return;
            }

            NetworkInventoryManager.Instance?.SendLootRequest(request);
        }
