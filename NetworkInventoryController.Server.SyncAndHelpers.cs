...
namespace Arawn.GameCreator2.Networking.Inventory
{
    ...
    public partial class NetworkInventoryController
    {
        ...
        private void ApplyFullSnapshot(NetworkInventorySnapshot snapshot)
        {
            ...
            int localItemsBefore = m_Bag.Content.CountWithStack;
            int trackedItemsBefore = m_RuntimeItemMap.Count;
            int snapshotCells = snapshot.Cells?.Length ?? 0;
            int snapshotStackItems = CountSnapshotStackItems(snapshot.Cells);

            LogPickupDebug(
                $"{name}: applying full snapshot bag={NetworkId} snapshotBag={snapshot.BagNetworkId} world={IsWorldInventory} server={m_IsServer} local={m_IsLocalClient} remote={m_IsRemoteClient} localItemsBefore={localItemsBefore} trackedBefore={trackedItemsBefore} snapshotCells={snapshotCells} snapshotStackItems={snapshotStackItems} firstCells={DescribeSnapshotCells(snapshot.Cells, 5)}",
                this);
            ...
            // [LOCAL-EDIT] #PILFER-INVENTORY-SNAPSHOT-DEBUG
            LogPickupDebug(
                $"{name}: applied full snapshot bag={NetworkId} localItemsAfter={m_Bag.Content.CountWithStack} trackedAfter={m_RuntimeItemMap.Count} snapshotCells={snapshotCells} snapshotStackItems={snapshotStackItems}",
                this);
        }

        // [LOCAL-EDIT] #PILFER-INVENTORY-SNAPSHOT-DEBUG
        private static int CountSnapshotStackItems(NetworkCell[] cells)
        {
            ...
            {
                count += Mathf.Max(0, cells[i].StackCount);
            }
        ...
        // [LOCAL-EDIT] #PILFER-INVENTORY-SNAPSHOT-DEBUG
        private static string DescribeSnapshotCells(NetworkCell[] cells, int maxCells)
        {
            if (cells == null || cells.Length == 0) return "none";

            int limit = Mathf.Min(cells.Length, Mathf.Max(0, maxCells));
            string description = string.Empty;
            for (int i = 0; i < limit; i++)
            {
                NetworkCell cell = cells[i];
                if (i > 0) description += "; ";
                description += $"#{i} pos={cell.Position} item={cell.RootItem.ItemIdString} runtime={cell.RootItem.RuntimeIdHash} stack={cell.StackCount}";
            }

            if (cells.Length > limit)
            {
                description += $"; +{cells.Length - limit} more";
            }
